...
import { PutProjectsProjectIdSharingsSharingIdServerList } from "../models/operations/putprojectsprojectidsharingssharingid.js";
...
> {
  ...
  const body = encodeJSON("body", payload.UpdateSharingRequest, {
    explode: true,
  });

  const baseURL = options?.serverURL
    || pathToFunc(PutProjectsProjectIdSharingsSharingIdServerList[0], {
  ...
  const pathParams = {
    ...
    "sharing-id": encodeSimple("sharing-id", payload["sharing-id"], {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/projects/{project-id}/sharings/{sharing-id}")(
    pathParams,
  );
  ...
  const context = {
    ...
    baseURL: baseURL ?? "",
    operationID: "put_/projects/{project-id}/sharings/{sharing-id}",
    oAuth2Scopes: [],
