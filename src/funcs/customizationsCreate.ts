...
import { PostMediasMediaIdCustomizationsServerList } from "../models/operations/postmediasmediaidcustomizations.js";
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.PostMediasMediaIdCustomizationsRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON("body", payload.VideoCustomization, {
    explode: true,
  });

  const baseURL = options?.serverURL
    || pathToFunc(PostMediasMediaIdCustomizationsServerList[0], {
  ...
  const context = {
    ...
    baseURL: baseURL ?? "",
    operationID: "post_/medias/{media-id}/customizations",
    oAuth2Scopes: [],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, models.VideoCustomizationResponse$inboundSchema),
