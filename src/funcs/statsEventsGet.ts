...
import { GetStatsEventsEventKeyServerList } from "../models/operations/getstatseventseventkey.js";
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetStatsEventsEventKeyRequest$outboundSchema.parse(value),
  ...
  const baseURL = options?.serverURL
    || pathToFunc(GetStatsEventsEventKeyServerList[0], {
      charEncoding: "percent",
    })();

  const pathParams = {
    "event-key": encodeSimple("event-key", payload["event-key"], {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/stats/events/{event-key}")(pathParams);
  ...
  const context = {
    ...
    baseURL: baseURL ?? "",
    operationID: "get_/stats/events/{event-key}",
    oAuth2Scopes: [],
