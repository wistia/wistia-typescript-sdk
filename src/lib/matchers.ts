...
import { WistiaDefaultError } from "../models/errors/wistiadefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | WistiaDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | WistiaDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new WistiaDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new WistiaDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
