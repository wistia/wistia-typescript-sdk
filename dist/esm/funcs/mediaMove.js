 ...
 * ```
 */
export function mediaMove(client, request, options) {
    return new APIPromise($do(client, request, options));
}
async function $do(client, request, options) {
    const parsed = safeParse(request, (value) => operations.PutMediasMoveRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const [result] = await M.match(M.json(200, operations.PutMediasMoveResponse$inboundSchema), M.json(207, operations.PutMediasMoveResponse$inboundSchema), M.jsonErr(400, errors.PutMediasMoveBadRequestError$inboundSchema), M.jsonErr(401, errors.PutMediasMoveUnauthorizedError$inboundSchema), M.jsonErr(404, errors.PutMediasMoveNotFoundError$inboundSchema), M.jsonErr(500, errors.PutMediasMoveInternalServerError$inboundSchema), M.fail("4XX"), M.fail("5XX"))(response, req, { extraFields: responseFields });
    if (!result.ok) {
        return [result, { status: "complete", request: req, response }];
    }
    return [result, { status: "complete", request: req, response }];
}
//# sourceMappingURL=mediaMove.js.map