 ...
 * ```
 */
export function mediaGet(client, request, options) {
    return new APIPromise($do(client, request, options));
}
async function $do(client, request, options) {
    const parsed = safeParse(request, (value) => operations.GetMediasMediaHashedIdRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const [result] = await M.match(M.json(200, operations.GetMediasMediaHashedIdResponse$inboundSchema), M.jsonErr(401, errors.GetMediasMediaHashedIdUnauthorizedError$inboundSchema), M.jsonErr(404, errors.GetMediasMediaHashedIdNotFoundError$inboundSchema), M.jsonErr(500, errors.GetMediasMediaHashedIdInternalServerError$inboundSchema), M.fail("4XX"), M.fail("5XX"))(response, req, { extraFields: responseFields });
    if (!result.ok) {
